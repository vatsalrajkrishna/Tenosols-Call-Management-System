...
$sql ="SELECT EmpId FROM tbluploader WHERE EmpId=:empid";
...
if($query->rowCount() > 0)
{
echo "<span style='color:red'> Uploader id already exists .</span>";
 echo "<script>$('#add').prop('disabled',true);</script>";
} else{
	
echo "<span style='color:green'> Uploader id available for Registration .</span>";
...
$empid= $_POST["emailid"];
$sql ="SELECT EmailId FROM tbluploader WHERE EmailId=:emailid";
$query= $dbh -> prepare($sql);
